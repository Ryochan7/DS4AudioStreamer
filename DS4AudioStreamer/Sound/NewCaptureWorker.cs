...
namespace DS4AudioStreamer.Sound
{
    public class NewCaptureWorker
    {
        ...
        private SbcAudioStream _stream;
        ...
        public NewCaptureWorker(
            ...
            Task.Run(() =>
            {
                // Have object affinity be outside main thread
        ...
        private unsafe void _worker()
        {
            ...
            unchecked
            {
                while (true)
                {
                    _stream.audioDataReadyEvent.Wait();

                    _stream.testLocker.EnterWriteLock();

                    _stream.audioDataReadyEvent.Reset();
                    ...
                    _stream.testLocker.ExitWriteLock();
                }
            }
        }
